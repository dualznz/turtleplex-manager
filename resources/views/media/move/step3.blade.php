            ...
            <div class="col-lg-12">
                ...
                <div class="card m-b-30">
                    ...
                    <div class="card-body">
                        ...
                        <form class="form-horizontal" method="POST" action="{{ route('media-move-step3-store', [$server->slug, $drive->slug, $media->media_type, $media->slug, $media->release_year]) }}">
                            @csrf
                            ...
                            <div class="form-group progress">
                                <div class="progress-bar progress-bar-striped progress-bar-animated progress-bar-info" style="width: 80%" role="progressbar" aria-valuenow="80" aria-valuemin="0" aria-valuemax="100"></div>
                            </div>
                            ...
                            <div class="form-group {{ $errors->first('Destination Folder') ? 'has-error' : ''}}">
                                <div class="row">
                                    <label class="col-lg-2 control-label label-right my-auto">Destination Folder</label>
                                    <div class="col-lg-3">
                                        <select class="form-control" name="destination_folder" id="destination_folder" required>
                                            <option selected disabled>Please select destination folder...</option>
                                            @foreach (\App\DriveAssets::where('server_id', $new_server->id)->where('drive_id', $new_drive->id)->get() as $s)
                                                <option value="{{ $s->id }}">{{ $s->asset_folder }}</option>
                                            @endforeach
                                        </select>
                                        <div class="help-block margin-bottom-none">
                                            <small>Please select the folder to add this media to.</small>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                <div class="row">
                                    ...
                                    <div class="col-lg-5">
                                        <a href="{{ route('media-move-step2', [$server->slug, $drive->slug, $media->media_type, $media->slug, $media->release_year]) }}" class="btn btn-success"><i class="far fa-chevron-double-left"></i>&nbsp; Go Back</a>&nbsp;&nbsp;
